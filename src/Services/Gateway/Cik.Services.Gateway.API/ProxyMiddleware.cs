...
namespace Cik.Services.Gateway.API
{
  public class ProxyMiddleware
  {
    ...
    public async Task Invoke(HttpContext context)
    {
      ...
      foreach (var removedPattern in _options.RemovedPatterns)
      {
        pathBase = pathBase.Replace(removedPattern, @"/");
        path = path.Replace(removedPattern, @"/");
      }

      var uriString =
        $"{_options.Scheme}://{_options.Host}:{_options.Port}{pathBase}{path}{context.Request.QueryString}";
      requestMessage.RequestUri = new Uri(uriString);
      requestMessage.Method = new HttpMethod(context.Request.Method);
      using (
        var responseMessage =
          await _httpClient.SendAsync(requestMessage, HttpCompletionOption.ResponseHeadersRead, context.RequestAborted))
      {
